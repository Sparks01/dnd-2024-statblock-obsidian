...
Restored Original Logic + Settings Toggle
*/
...
class DnDStatblockSettingTab extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl('h2', { text: 'D&D 2024 Statblock Settings' });
    new import_obsidian.Setting(containerEl)
      .setName('Retro / Cassette Futurism Mode')
      .setDesc('Toggle this ON to transform statblocks into the VHS/Sci-Fi style.')
      .addToggle(toggle => toggle
        .setValue(this.plugin.settings.retroMode)
        .onChange(async (value) => {
          this.plugin.settings.retroMode = value;
          await this.plugin.saveSettings();
          this.plugin.updateBodyClass();
        }));
  }
}
...
var DnD2024StatblockPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.addSettingTab(new DnDStatblockSettingTab(this.app, this));
  ...
  onunload() {
    ...
    document.body.classList.remove("dnd-retro-mode");
  }
  ...
  updateBodyClass() {
    if (this.settings.retroMode) {
      document.body.classList.add("dnd-retro-mode");
    } else {
      document.body.classList.remove("dnd-retro-mode");
    }
  }

  // --- ORIGINAL HELPER FUNCTIONS RESTORED BELOW ---
  ...
  // Define section categories with priorities
  const sectionCategories = {
    // Must stay in right column (combat actions)
    rightFixed: {
      patterns: [/^actions$/i, /^bonus actions$/i, /^legendary actions$/i, /^reactions$/i],
      priority: 'right'
    },
    // Prefer left column (character traits)
    leftPreferred: {
      patterns: [/^traits$/i, /^features$/i, /^spellcasting$/i],
      priority: 'left'
    },
    // Can go anywhere for balance
    flexible: {
      patterns: [/^spells$/i, /^equipment$/i, /^description$/i, /^lair actions$/i, /^regional effects$/i],
      priority: 'flexible'
    }
  };
  ...
  const calculateContentLength = (content) => {
    // Remove markdown formatting for more accurate length
  ...
  // Categorize and measure all sections
  const categorizedSections = sections.map(section => {
    let category = 'flexible';
    const title = section.title.toLowerCase();
    
    // Determine category
    for (const [catName, catData] of Object.entries(sectionCategories)) {
      if (catData.patterns.some(pattern => pattern.test(title))) {
        category = catName;
        break;
      }
    }
    ...
    return {
      ...section,
      category,
      length: calculateContentLength(section.content),
      priority: sectionCategories[category]?.priority || 'flexible'
    };
  });

  // Separate sections by priority
  const rightFixed = categorizedSections.filter(s => s.priority === 'right');
  const leftPreferred = categorizedSections.filter(s => s.priority === 'left');
  const flexible = categorizedSections.filter(s => s.priority === 'flexible');

  // Calculate fixed content lengths
  const rightFixedLength = rightFixed.reduce((sum, s) => sum + s.length, 0);
  const leftPreferredLength = leftPreferred.reduce((sum, s) => sum + s.length, 0);

  // Start with fixed sections
  const leftSections = [...leftPreferred];
  const rightSections = [...rightFixed];
  
  let leftTotal = leftPreferredLength;
  let rightTotal = rightFixedLength;

  // Distribute flexible sections to balance columns
  // Sort flexible sections by length (largest first) for better distribution
  flexible.sort((a, b) => b.length - a.length);

  for (const section of flexible) {
    // Calculate what the imbalance would be if we add to each column
    const leftImbalanceIfAdded = Math.abs((leftTotal + section.length) - rightTotal);
    const rightImbalanceIfAdded = Math.abs(leftTotal - (rightTotal + section.length));

    // Add to whichever column results in better balance
    if (leftImbalanceIfAdded <= rightImbalanceIfAdded) {
      leftSections.push(section);
      leftTotal += section.length;
    } else {
      rightSections.push(section);
      rightTotal += section.length;
    }
  }

  // Maintain original order within each column
